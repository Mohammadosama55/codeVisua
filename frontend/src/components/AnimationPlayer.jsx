...
import { Play, Pause, SkipBack, SkipForward, StickyNote, Check, X } from 'lucide-react';
          ...
          <div key={i} className="flex items-center">
            ...
            {i < nodes.length - 1 && (
              <div className="flex items-center mx-1">
                ...
                <div className="w-0 h-0" style={{ borderWidth: '4px 0 4px 6px', borderColor: 'transparent transparent transparent #6b7280', borderStyle: 'solid' }} />
              </div>
            )}
          </div>
        );
      ...
      <svg width={260} height={200} className="overflow-visible">
        {nodes.map((_, i) =>
          nodes.slice(i + 1).map((__, j) => (
            <line key={`${i}-${i+1+j}`} x1={positions[i].x} y1={positions[i].y} x2={positions[i+1+j].x} y2={positions[i+1+j].y} stroke="#374151" strokeWidth={1.5} />
          ))
        )}
        {nodes.map((val, i) => {
          ...
          return (
            <g key={i}>
              <motion.circle cx={positions[i].x} cy={positions[i].y} r={20} animate={{ fill: active ? '#6d28d9' : '#1f2937' }} stroke={active ? '#8b5cf6' : '#4b5563'} strokeWidth={2} />
...
function Viz({ type, data, highlight }) {
  if (!data || (Array.isArray(data) && data.length === 0)) {
    return <div className="flex items-center justify-center h-52 text-gray-600 text-sm">No data to visualise</div>;
  }
...
/* ─── inline step note ─── */
function StepNote({ stepId, getNote, setNote }) {
  const [editing, setEditing] = useState(false);
  const [draft,   setDraft]   = useState('');
  const saved = getNote(stepId);

  const open = () => { setDraft(saved); setEditing(true); };
  const save = () => { setNote(stepId, draft); setEditing(false); };
      ...
      {!editing ? (
        <button
          ...
          className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg border transition-all ${
            saved ? 'bg-yellow-950/50 border-yellow-700 text-yellow-300' : 'bg-gray-900 border-gray-700 text-gray-500 hover:text-yellow-400 hover:border-yellow-700'
          }`}
        >
          <StickyNote className="w-3 h-3" />
          {saved ? 'My note (click to edit)' : 'Add a note to this step'}
        </button>
      ) : (
        <AnimatePresence>
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-1.5">
            <textarea
              ...
              rows={2}
              className="w-full bg-yellow-950/30 border border-yellow-700 rounded-lg px-3 py-2 text-yellow-100 text-xs resize-none outline-none placeholder-yellow-800 focus:border-yellow-500"
            />
            <div className="flex gap-1.5">
              <button onClick={save}   className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-yellow-700 hover:bg-yellow-600 text-white text-xs font-semibold transition-colors"><Check className="w-3 h-3" />Save</button>
              <button onClick={cancel} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-400 text-xs transition-colors"><X className="w-3 h-3" />Cancel</button>
              {saved && <button onClick={() => { setNote(stepId, ''); setEditing(false); }} className="px-2.5 py-1 rounded-lg bg-red-950 hover:bg-red-900 text-red-400 text-xs transition-colors">Delete</button>}
            </div>
          </motion.div>
        </AnimatePresence>
      )}
      {saved && !editing && (
        <p className="mt-1 text-xs text-yellow-400/80 bg-yellow-950/30 border border-yellow-800/50 rounded-lg px-3 py-1.5 leading-relaxed">{saved}</p>
      )}
    </div>
  );
}
...
/* ─── main player ─── */
export default function AnimationPlayer({ animation, topic, getNote, setNote }) {
  ...
  useEffect(() => { setCurrentStep(0); setPlaying(false); }, [animation]);
  ...
  if (!animation || totalSteps === 0) {
    return <div className="bg-gray-900 rounded-2xl border border-gray-800 flex items-center justify-center h-64 text-gray-500">No animation data available.</div>;
  }
    ...
    <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
      ...
      <div className="min-h-56 flex items-center justify-center px-4 py-2 bg-gray-950/40">
        <AnimatePresence mode="wait">
          <motion.div key={currentStep} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.02 }} transition={{ duration: 0.2 }} className="w-full">
            <Viz type={animation.type} data={vizData} highlight={step.highlight} />
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="px-6 py-4 border-t border-gray-800 bg-gray-950">
        <AnimatePresence mode="wait">
          <motion.div key={currentStep} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="mb-3">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[11px] text-violet-400 font-mono bg-violet-950/50 px-2 py-0.5 rounded-full">{currentStep + 1} / {totalSteps}</span>
              <span className="text-white font-semibold text-sm">{step.title}</span>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed">{step.description}</p>
            {step.state && (
              <p className="text-gray-500 text-xs mt-1.5 font-mono bg-gray-900 px-2.5 py-1 rounded-lg inline-block border border-gray-800">{step.state}</p>
            )}
            {/* Per-step notes */}
            {getNote && setNote && (
              <StepNote stepId={`step-${currentStep}`} getNote={getNote} setNote={setNote} />
            )}
          </motion.div>
        </AnimatePresence>
        ...
        <div className="mb-4 h-1.5 bg-gray-800 rounded-full overflow-hidden">
          <motion.div animate={{ width: `${((currentStep + 1) / totalSteps) * 100}%` }}
            className="h-full bg-gradient-to-r from-violet-600 to-blue-500 rounded-full" transition={{ duration: 0.3 }} />
        </div>

        {/* controls */}
        <div className="flex items-center justify-center gap-3">
          <button onClick={() => { setCurrentStep(0); setPlaying(false); }} title="Restart"
